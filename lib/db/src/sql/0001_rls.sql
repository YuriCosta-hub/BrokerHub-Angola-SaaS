...
-- Políticas usam set_config('app.tenant_id' | 'app.clerk_user_id' | 'app.allow_tenant_create' | 'app.rls_bypass', ..., true)
-- dentro de transacções da API (is_local = true). FORCE aplica-se também ao dono da tabela (o role da DATABASE_URL).
-- app.rls_bypass só é definido por workers internos — nunca a partir de HTTP.
...
CREATE POLICY tenants_insert ON tenants
  ...
  WITH CHECK (
    current_setting('app.rls_bypass', true) = 'on'
    OR current_setting('app.allow_tenant_create', true) = 'on'
  );
